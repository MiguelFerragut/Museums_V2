...
router.post('/create', isLoggedIn, checkRole('GUIDE', 'ADMIN'), uploaderMiddleware.single('cover'), (req, res, next) => {
    ...
    const { title, description, day, duration, departments, language } = req.body
    ...
    Event
        .create({ title, guideName, description, cover, date, departments, language })
...
router.get("/edit/:event_id", isLoggedIn, checkRole('GUIDE', 'ADMIN'), (req, res, next) => {
    ...
    const promises = [
        Event.findById(event_id).populate('guideName').populate('departments'),
        Department.find().select({ name: 1 })
    ]

    return Promise.all(promises)
        .then(([event, departments]) => {
            res.render('events/edit', { event, departments })
        })
        .catch(err => next(err))
})
...
router.post('/edit/:event_id', isLoggedIn, checkRole('GUIDE', 'ADMIN'), (req, res, next) => {
    ...
    const { event_id } = req.params
    const { title, guideName, description, cover, date, participants, departments, language } = req.body

    Event
        .findByIdAndUpdate(event_id, { title, guideName, description, cover, date, participants, departments, language })
        .then(() => res.redirect('/events/list'))
        .catch(err => next(err))
})
